...
if 'scope_input' not in st.session_state: st.session_state.scope_input = ""
...
def extract_app_fields(file):
    ...
    if "是否存在外包过程" in full_text:
        ...
        if any(x in out_part for x in ["■是", "☑是", "√是"]):
            is_outsourced = "是"
        elif any(x in out_part for x in ["■否", "☑否", "√否"]):
            is_outsourced = "否"
        else:
            is_outsourced = "未清晰勾选"
    ...
    if "QMS:" in full_text_original and "EMS:" in full_text_original:
        try:
            scope_content = full_text_original.split("QMS:")[1].split("EMS:")[0].strip()
        except:
            pass
...
def extract_manual_sections(file):
    ...
    for p in doc.paragraphs:
        ...
        elif "8.1.5.3" in text and "外包" in text:
            current_section = "outsource_8153"
        elif current_section and re.match(r"^\d+\.\d+", text) and not text.startswith("4.") and not text.startswith(
                "8."):
            current_section = None
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            if "8.3" in row_no_space and (
                    "设计" in row_no_space or "开发" in row_no_space or "▲" in row_no_space or "●" in row_no_space or "○" in row_no_space):
...
if all(os.path.exists(f) for f in [CODE_FILE, CALC_FILE, CAT_FILE, RISK_FILE]):
    ...
    with col1:
        st.text_area("✍️ 请输入受审核方的范围描述：", height=120, placeholder="例如：电子元器件的生产...",
                     key="scope_input")
    ...
    if license_file is not None:
        with st.spinner("📷 正在 OCR 识别执照并比对经营范围..."):
            ...
            check_prompt = f"""
                    ...
                    """
            try:
                res_lic = client.chat.completions.create(model="deepseek-v4-pro",
        ...
        for code in candidate_codes:
            with st.expander(f"📍 候选代码 【{code}】 的完整配置方案", expanded=True):
                ...
                if not risk_info.empty:
                    q_level = num_to_text.get(int(risk_info.iloc[0, 1]), "中") if pd.notna(
                        risk_info.iloc[0, 1]) else "中"
                    e_level = num_to_text.get(int(risk_info.iloc[0, 2]), "中") if pd.notna(
                        risk_info.iloc[0, 2]) else "中"
                    s_level = num_to_text.get(int(risk_info.iloc[0, 3]), "中") if pd.notna(
                        risk_info.iloc[0, 3]) else "中"
                else:
...
for i in range(3):
    ...
    with r_col1:
        input_scope = st.text_input(f"范围 {i}", label_visibility="collapsed", key=f"s_{i}", placeholder="范围")
    with r_col2:
        input_code = st.text_input(f"代码 {i}", label_visibility="collapsed", key=f"c_{i}", placeholder="代码")
    if input_scope.strip() and input_code.strip(): batch_data.append(
        {"scope": input_scope.strip(), "code": input_code.strip()})
