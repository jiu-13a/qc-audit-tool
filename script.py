...
if all(os.path.exists(f) for f in [CODE_FILE, CALC_FILE, CAT_FILE]):
    ...
    if st.button("🚀 确定代码与人员"):
        if not scope.strip():
            ...
            # AI 判定逻辑
            search_key = scope[:10].replace("生产", "").replace("销售", "").strip()
            mask = df_code.apply(lambda row: row.astype(str).str.contains(search_key).any(), axis=1)
            candidates = df_code[mask].head(30).to_string()
            exp_context = df_exp.tail(5).to_string()  # 参考最近5条经验

            prompt = f"""
你现在是【北京北方启辰认证服务有限公司】的技术中心首席评审专家，负责管理体系（QMS/EMS/OHSMS）的专业类别判定。
...
### 3. 判定执行逻辑（必须按此步骤思考）
1. **核心业务识别**：分析经营范围中的动词（制造、施工、服务、研发）和名词（产品或行业），确定其所属的 CNAS 大类（如：07 机械设备、28 建设、35 信息技术）。
2. **候选匹配与排查**：
    - 将业务与候选代码的“包含项”进行比对。
    - **重点检查“不包括”项**：如果业务中包含某些关键词，但该代码明确声明“不包括”此类业务，必须立即排除。
3. **专业性校验**：判断该业务是否涉及高风险、专业分包或特定行政许可。

### 4. 输出格式要求（严禁改动标题）
- **初步分析**：[简述该企业的核心行业属性及业务特征]
- **排除过程**：[说明你排除了哪些容易混淆的代码，并引用“不包括”项作为证据]
- **最终建议代码**：[6位小类代码，例如 14.01.01]
- **分类名称**：[官方标准名称]
- **判定理由**：[结合经营范围，详细说明为什么该代码是最精准的匹配]
"""f"""
你现在是【北京北方启辰认证服务有限公司】的首席评审专家。
任务：根据《经营范围》判定最精准的6位小类代码。
...
### 要求：
1. 必须给出6位代码（如 29.01.01）。
2. 必须核对“不包括”项。
3. 请在回答的最后一行，仅输出代码号，格式为：最终代码：[代码号]
