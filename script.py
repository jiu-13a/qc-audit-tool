		...
		if not scope_input.strip():
			...
			# === 阶段 3：如果经验库没找到，走 AI 判定 ===
				...
				with st.spinner("🤖 未命中经验库，AI 正在全库检索审核代码..."):
					...
					prompt = f"""
你是一位精通管理体系认证机构认证业务范围分类指南的专业技术专家。你的核心任务是精准提取并匹配“专业代码”。

### 1. 输入数据
- **待评审范围**：{scope_input}
- **历史判定经验**：{exp_context}
- **全量代码库**：{code_library_text}

### 2. 核心执行逻辑（必须按顺序执行）
**第一步：语义拆解（关键）**
将《待评审范围》拆解为独立的“原子项”。例如：“玻璃仪器、石英仪器、化工原料的销售”应拆解为：
1. 玻璃仪器 + 销售
2. 石英仪器 + 销售
3. 化工原料 + 销售
请确保没有任何一个名词或动作被遗漏。
...
**第二步：代码检索与比对**
针对拆解后的每一个“原子项”，在【全量代码库】中进行检索。
- **优先参考经验**：如果原子项与《历史判定经验》高度相似，必须优先采用。
- **深度区分**：仔细核对“包含项”与“不包括”项，防止化工类代码误选入危险品范围。

**第三步：覆盖度自检**
在输出前自我询问：我给出的代码方案是否涵盖了《待评审范围》中提到的**所有**产品和活动？如果有遗漏（如玻璃、石英），必须重新补全。

### 3. 判定要求
- **严禁幻觉**：仅能使用代码库中存在的真实代码。
- **多候选输出**：按匹配度排序。如果一个原子项可能对应多个代码，请全部列出并说明差异。
- **固定提取格式**：每个匹配项必须包含：建议代码：[6位代码号]

### 4. 输出格式（严格遵守）
#### [原子项 A 的匹配结果]
- **匹配代码**：[建议代码：XX.XX.XX]
- **分类名称**：[名称]
- **依据说明**：[简述理由，并对照“包含/不包含”项说明]

#### [原子项 B 的匹配结果]
...（以此类推，确保全覆盖）

### 5. 特殊指令
如果范围中包含“销售/贸易/批发/零售”等动作，请务必确认是否应归入第 29 类（批发和零售业）或相关制造类代码，并注明区分依据。
"""
					try:
